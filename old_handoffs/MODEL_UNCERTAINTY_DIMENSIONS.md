# Model-uncertainty program — canonical docs (dissolved 2026-08-21)

experiments/model_error/ held no callers once objectives.py -> core/hard_objectives.py,
worlds.py -> core/worlds.py and failure/steady_metrics.py -> dashboard/. Its unit
battery is now unit_tests/test_objectives_unit.py. These two docs are the record.
...
- `rerank/<tag>/seed_<s>.json` (rerank_ladder.py): per-seed
  ...
  popp_fail{mean,max}}}}`.
- `hardB3_scores.json` (dashboard/score_hardb3.py): see
  dashboard/README.md.
