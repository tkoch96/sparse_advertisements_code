# UG-sharding removal survey (dissolved 2026-08-21)

NOT deployment/latency sharding. This was about UG slices shipped to workers.
The survey's recommendation landed: split_deployment_by_ug is gone from
helpers/helpers.py, and the 08-19 commit stripped ~404 lines (143 from
worker_comms alone). prove_inert.py, the bitwise check that the removal
changed nothing, is in old_scripts/.

Deployment/latency sharding is a SEPARATE, live concept: core/shard_loader.py
+ core/convert_latencies.py, opt-in via SCULPTOR_LAT_SHARDS.
