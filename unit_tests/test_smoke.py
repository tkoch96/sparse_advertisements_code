...
def test_worker_handle_msg_dispatches(worker):
	"""handle_msg is the shim Worker_Manager uses. Confirm it routes a
...
def test_worker_unknown_cmd_does_not_crash(worker):
	...
	# Unknown-cmd still returns the bare sentinel; a cmd that RAISES returns
	# "ERROR\n<traceback>" (2026-08-21), so callers must treat it as a prefix.
