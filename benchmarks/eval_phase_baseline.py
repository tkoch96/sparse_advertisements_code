"""Per-phase timing + crash diagnostics for eval_latency_failure.
...
# Run_ray.py aliases worker_comms → worker_comms_ray; mirror that here so the
...
PHASE_KEYS = [
    ...
    'volume_multipliers',        # 20 global-volume points
...
PHASE_HEADER_PREFIXES = {
    ...
    '-----Volume multiplier calc for deployment number': 'volume_multipliers',
...
def main():
    ...
    from wrapper_eval import global_performance_metrics_fn, global_soln_types
    ...
    vol_mult_fields = ['volume_multipliers']
    ...
    metrics_summary['volume_multipliers'] = _summarise_fields(vol_mult_fields)
    ...
    print(f"volume_multipliers populated: {metrics_summary['volume_multipliers']}")
