...
# https://github.com/TopologyMapping/bgpsim
import bz2
...
class PathPref(enum.IntEnum):
    ...
    def from_relationship(graph: ASGraph, exporter: int, importer: int):
        ...
        raise ValueError("Unsupported relationship {}".format(rel))
...
class ASGraph:
    ...
    def read_caida_asrel_graph(filepath):
        ...
        return graph
    @staticmethod
    def manual_graph_input(as_relationships):
        graph = ASGraph()
        for (source,sink), rel in as_relationships.items():
