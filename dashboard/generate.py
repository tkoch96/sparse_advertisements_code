...
if REPO not in sys.path:            # so `python dashboard/generate.py` works
    sys.path.insert(0, REPO)
SITE = os.path.join(REPO, 'dashboard_site')

# Cluster runs are DISCOVERED, not registered: anything cluster/expctl.py
# has launched and harvested gets a tab section automatically. See
# dashboard/cluster_runs.py.
from dashboard import cluster_runs  # noqa: E402
NS = [1, 2, 5, 10, 20, 50]
    ...
    {'id': 'ablation_v3lbc', 'title': 'Policy ladder: LB cache ON',
     'sections': [
         {'id': 'ladder_v3lbc', 'title': '7-arm ladder v3',
          ...
          'progress_manifest': 'cluster/manifests/v3lbc_manifest.json',
          'progress_live': False,
    ...
    {'id': 'ablation_v3', 'title': 'Ablation: policy ladder',
     'sections': [
         {'id': 'ladder_v3', 'title': '7-arm ladder v3',
          ...
          'progress_manifest': 'cluster/manifests/v3grid_manifest.json',
    ...
    {'id': 'ladder_a10', 'title': 'Ladder: actual-10',
     'sections': [
         {'id': 'a10', 'title': 'L1-L6 @ actual-10', 'kind': 'ladder_links',
          ...
          'figs_url': 'figs_a10',
          'fixed_all_n': True,
          'progress_manifest': 'cluster/manifests/a10_manifest.json',
          'progress_live': False,
          'arms': [
              ('fixed', 'no_mc', 'fixed', 'L1 no_mc+fixed',
...
# ---- NEW SOLVER (HiGHS) tabs — Tom 2026-08-17 ----
# Same grid as the v4 era, LPs through core/gpshim.py
...
EXPERIMENTS.extend([
    {'id': 'ladder_highs', 'title': 'Policy ladder — HiGHS',
     'sections': [
         {'id': 'ladder_hx', 'title': 'L1-L6 (new solver)',
          ...
          'heading': 'Policy ladder — NEW SOLVER (HiGHS) '
    ...
    {'id': 'hardobj_highs', 'title': 'Hard objectives — HiGHS',
     'sections': [
         {'id': 'overview', 'title': 'overview', 'kind': 'static',
          'progress_manifest': 'tools/v4grid_manifest_highs.json',
          'intro': ('<p>NEW SOLVER (HiGHS): fracb / mlu / '
...
EXPERIMENTS.extend([
    {'id': 'grid_maxhard_v2', 'title': 'Unified grid (maxhard v2, seeds 201-205, obj-round 4dp)',
     'sections': [
         {'id': 'overview', 'title': 'overview', 'kind': 'static',
          'progress_manifest': 'cluster/manifests/grid_maxhard_v2_manifest.json',
...
EXPERIMENTS.append({
    ...
    'sections': [
        {'id': 'overview', 'title': 'overview', 'kind': 'static',
         'progress_manifest': 'cluster/manifests/grid_maxhard_v2_gurobi_manifest.json',
         'progress_live': False,
...
EXPERIMENTS.append({
    ...
    'sections': [
        {'id': 'a10x10', 'title': 'L1-L6 @ actual-10, 10 deployments',
         ...
         'progress_manifest': 'cluster/manifests/a10x10_manifest.json',
...
EXPERIMENTS.append({
    ...
    'sections': [
        {'id': 'overview', 'title': 'overview', 'kind': 'static',
         'progress_manifest': 'cluster/manifests/grid_v5scout_manifest.json',
         ...
         'refresh': {'steps': [
             {'in': ['figures/dashboards/adagrad_smoke/adagrad_smoke_grads.png'], 'always': True,
              'out': ['figures/dashboards/adagrad_smoke/adagrad_smoke_grads.png',
                      'figures/dashboards/adagrad_smoke/adagrad_smoke_obj.png'],
              'argv': ['{py}', '-m',
                       'dashboard.plot_adagrad_smoke']},
         ]}},
    ]})

# ---- EODS-25 (Tom 2026-08-19: SCULPTOR-full/L6 at dpsize 25 through
# the classical eval_all_solution_types metrics; queue-native cells on the
    ...
    'sections': [
        {'id': 'overview', 'title': 'overview', 'kind': 'static',
         'heading': 'evaluate_over_deployment_sizes @ actual-25 — '
                    ...
                    'HiGHS, classical eval_all_solution_types metrics '
             ...
             'steps': [
                 {'in': ['cache/eods/v1_dash/*'], 'always': True,
                  'out': ['figures/dashboards/eods25/eods25_status.png',
                          'figures/dashboards/eods25/eods25_results.png'],
                  'argv': ['{py}', '-m',
                           'dashboard.plot_eods25']},
                 {'in': ['cache/eods/v1_dash/*'], 'always': True,
                  'out': ['figures/dashboards/eods25/eods25_run.png'],
                  'argv': ['{py}', '-m',
                           'dashboard.plot_eods25_run']},
...
# First tab, because when a VM is up this is the only tab that matters.
# Built at import time from whatever expctl has harvested, so a run
...
def main():
    ...
    # 30s: the page is served from localhost and the cluster loop
    # regenerates every 30s, so this is the fastest cadence that can
    # actually show new data. Scroll position and tab are restored below,
    # which is what makes a reload this frequent tolerable to read through.
    refresh = os.environ.get('SCULPTOR_DASH_REFRESH_S', '30')
    html = TEMPLATE.replace('@NAV@', '\n'.join(nav)) \
                   .replace('@PANELS@', '\n'.join(panes)) \
                   .replace('@REFRESH@', refresh) \
                   .replace('@STAMP@', stamp)
    # Atomic replace: the fast cluster loop and the slow all-experiments
    # refresh loop both call this, and a plain truncate-then-write can be
    # read half-finished by the browser mid-poll.
...
// Scroll position survives the auto-reload. Without this a 30s refresh
// yanks you back to the top mid-read, which is why the old cadence had to
...
// One-click copy for the tail commands.
