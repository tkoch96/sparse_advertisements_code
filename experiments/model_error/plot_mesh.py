...
RERANK_GLOBS = os.environ.get(
    'MESH_PLOT_RERANK', 'cache/model_error/rerank/mesh_N*').split(',')
STEADY_FILES = os.environ.get(
    'MESH_PLOT_STEADY', 'cache/model_error/steady/mesh_steady.json').split(',')
OUT_NAME = os.environ.get('MESH_PLOT_OUT', 'mesh_georand_overN')


def load():
    pfail = {}
    for g in RERANK_GLOBS:
        for fn in glob.glob(os.path.join(g, 'seed_*.json')):
    ...
    scong, clean = {}, {}
    entries = []
    for sf in STEADY_FILES:
        entries.extend(json.load(open(sf)))
    for e in entries:
        if not e.get('solved'):
            continue
...
def main():
    ...
    for ext in ('png', 'pdf'):
        fig.savefig('figures/{}.{}'.format(OUT_NAME, ext), dpi=170)
    print('wrote figures/{}.png'.format(OUT_NAME))
