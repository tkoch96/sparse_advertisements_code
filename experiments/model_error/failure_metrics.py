...
seeds run in parallel (5 Gurobi sessions).
    ...
    SCULPTOR_LAT_MODEL=geo SCULPTOR_PREF_MODEL=random \
        python -m experiments.model_error.failure_metrics \
...
# Solver-fork seam (Tom 2026-08-17): SCULPTOR_LP_BACKEND set => score
# through the gpshim-backed fork stack (see ablation/run_fork_ladder.py).
...
def run_seed(s, args):
    ...
    env.update({'RAY_ADDRESS': 'local',
                'RAY_TMPDIR': '/tmp/ray_failm_{}_{}'.format(
                    os.getpid(), s)})
    p = subprocess.run(
        [sys.executable, '-m', 'experiments.model_error.failure_metrics',
