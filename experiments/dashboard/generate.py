...
# ---- UNIFIED GRID (maxhard world) — Tom 2026-08-17: one dash, all ----
...
_GG_FIGS = 'cache/ablation/grid_maxhard_artifacts/figs'
...
def _gg_section(key, title, figure):
    return {'id': key, 'title': title, 'kind': 'ladder_links',
            'figs_dir': _GG_FIGS, 'figs_url': 'figs_gridmh',
            'fixed_all_n': True,
            'arms': _hx_arms('figs_gridmh', _GG_FIGS, key + '_'),
            'heading': '{} — unified grid, maxhard world, L1-L6, '
...
EXPERIMENTS.extend([
    {'id': 'grid_maxhard', 'title': 'Unified grid (maxhard)',
     'sections': [
         {'id': 'overview', 'title': 'overview', 'kind': 'static',
          'progress_manifest': 'tools/grid_maxhard_manifest.json',
          'intro': ('<p>ONE experimentation framework, every objective a '
                    ...
                    'AND scored in the maxhard world, HiGHS backend, '
                    'slotted WHEN from mainline '
                    'sparse_advertisements_v3.</p>'),
          'figures': ['figures/grid_maxhard_4panel.png'],
          'refresh': {
              'pull': [('cache/ablation/grid_maxhard/',
                        'cache/ablation/grid_maxhard/'),
                       (_GG_FIGS + '/', _GG_FIGS + '/')],
              'steps': [
                  {'in': ['cache/ablation/grid_maxhard/*/*/N*/'
                          'seed_*_*.json'],
                   'out': ['figures/grid_maxhard_4panel.png'],
                   'always': True,
                   'env': {'HARDOBJ_ROOT': 'cache/ablation/grid_maxhard',
                           'HARDOBJ_OUT_PREFIX': 'grid_maxhard',
                           'HARDOBJ_OBJS': _GG_OBJS},
                   'argv': ['{py}', '-m',
                            'experiments.dashboard.plot_hardobj_v4']},
              ]}},
         _gg_section('lat', 'latency + gamma*resilience',
                     'figures/grid_maxhard_lat.png'),
         _gg_section('fracb', 'frac_beyond_optimal',
                     'figures/grid_maxhard_fracb.png'),
         _gg_section('mlu', 'max_util v2', 'figures/grid_maxhard_mlu.png'),
         _gg_section('prio', 'joint priority',
                     'figures/grid_maxhard_prio.png'),
     ]},
])
...
def main():
    ...
    for name, target in (('figs', os.path.join(REPO, HARDB3_FIGS)),
                         ...
                         ('figs_hardhx', os.path.join(
                             ...
                             '/figs')),
                         ('figs_gridmh', os.path.join(
                             REPO,
                             'cache/ablation/grid_maxhard_artifacts'
                             '/figs')),
