...
from helpers import *
# (local) evaluate_all_metrics is imported but never used by the plotter; dropped so the
# flat-layout evaluator stack (sparse_advertisements_v3 etc.) is not needed here
...
def make_paper_plots(cache_fn):
	...
	####### SINGLE INGRESS/SITE FAILURES
	for metric_k, lab in zip(['popp', 'pop'], ['Link', 'Site']):
		f,ax = get_figure(l=3.5)   # (local, Tom 2026-09-10) same shape as the steady-state CDF; was the wide 7x3 default
		for solution in soln_types:
			all_differences, x, stats, threshold_stats = get_failure_metric_arr('{}_failures_latency_optimal_specific'.format(metric_k), solution)
			metrics['stats_' + '{}_failures_latency_optimal_specific'.format(metric_k)][solution] = stats
			metrics['stats_latency_thresholds_fail_{}'.format(metric_k)][solution] = threshold_stats
		...
		# ax.legend()   # (local, Tom 2026-09-10) legend lives in the steady-state figure to its left
		ax.grid(True)
		# (local, Tom 2026-09-15) x-axis text one point smaller than the 14pt default: the
		# long label was cut off at the 3.5in figure width in the paper
		ax.set_xlabel("Optimal - Achieved Latency (ms)\nAfter Single-{} Failure".format(lab), fontsize=13)   # two lines: one point smaller alone still overflowed
