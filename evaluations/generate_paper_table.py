...
def _tex_pair_columns(labels, rows, precs):
    """Fold registry-declared pairs into single 'left / right' columns.
    Returns (labels', rows', precs', headers): a paired column's cell is
    ('pair', left_cell, right_cell); its header is the declared text, with
    'latency (ms)' -> 'latency subopt (ms)' when the left fallback stood in."""
    ...
    for i, l in enumerate(labels):
        if i in pos:
            ...
            new_labels.append(l); new_precs.append(precs[i])
            for d in rows:
                new_rows[d].append(rows[d][i])
    return new_labels, new_rows, new_precs, headers
...
# Tom 2026-09-10 (tex only): columns rescaled to a 0..100 band between two
...
def _tex_normalized(labels, rows):
    """rows with TEX_NORMALIZE columns mapped onto 0..100 (monotone, so
    the best-in-column flags are unchanged); precision 1 for those."""
    out = {disp: list(cells) for disp, cells in rows.items()}
    precs = []
    for i, lab in enumerate(labels):
        ...
        v0 = rows.get(m0, [None] * len(labels))[i][0] if m0 in rows else None
        v100 = rows.get(m100, [None] * len(labels))[i][0] if m100 in rows else None
        if v0 is None or v100 is None or abs(v100 - v0) < 1e-12:
            continue
        for disp in out:
            mean, std, n, best = out[disp][i]
            if mean is None:
                continue
            out[disp][i] = (100.0 * (mean - v0) / (v100 - v0),
                            (100.0 * std / abs(v100 - v0)) if std is not None else None,
                            n, best)
        precs[-1] = 1
    return out, precs
...
def emit(labels, rows, fmt, out_dir, basename='paper_table'):
    ...
    if fmt in ('latex', 'all'):
        ...
        trows, tprecs = _tex_normalized(labels, rows)
        tlabels, trows, tprecs, pair_headers = _tex_pair_columns(labels, trows, tprecs)
        # regroup on the folded labels
        groups = []
        for l in tlabels:
            g = HEADER_TEXT.get(l.split('|')[0], l.split('|')[0])
            if groups and groups[-1][0] == g:
                ...
                groups.append([g, 1])
        subs = [l.split('|', 1)[1] for l in tlabels]
        ncols = 1 + len(tlabels)
        with open(pth, 'w') as f:
            ...
            f.write('\\begin{table*}[!t]\n\\centering\n'
                    ...
                    '\\resizebox{\\textwidth}{!}{%\n')
            # full grid (Tom 2026-09-10): a vertical rule around every
            ...
            # the bare label
            _esc = [_wrap_tex_header(pair_headers.get(l) or TEX_SUB_DISPLAY.get(
                        l, TEX_SUB_DISPLAY.get(x, x)), width=(18 if l in pair_headers else None))
                    for l, x in zip(tlabels, subs)]
...
def reemit_from_csv(csv_path, out_dir, paper_dir):
    """Rebuild (labels, rows) from an emitted paper_table*.csv and re-emit
    with the current display rules. Columns are re-ordered to the registry's
    current key/group order; the best-in-column rule is the emit rule
    (reference methods excluded, ties to SCULPTOR)."""
    import csv as _csv
    # the key table is a SELECTION of the full table: always read the full
    # CSV (the superset) so the key CSV can never drift (it did, 2026-09-10)
    ...
    for row in rd[2:]:
        if not row or not row[0]:
            continue
        means[row[0]] = [float(x) if x not in ('', 'nan') else None for x in row[1:]]
    # current registry order: key table order if every column is a key
    # column, else the full-table group order
    ...
    ordered = sorted(labels_csv, key=lambda l: (group_rank.get(l.split('|')[0], 99),
                                                col_rank.get(l, 99), all_labels.index(l)))
    ...
    idx = [all_labels.index(l) for l in ordered]
    rows = {disp: [] for _k, disp in METHODS}
    for l, i in zip(ordered, idx):
        col = {disp: (means.get(disp) or [None] * len(all_labels))[i] for _k, disp in METHODS}
        finite = {k: v for k, v in col.items() if v is not None and k not in REFERENCE_METHODS}
        best = None
        if len(finite) > 1:
            best = (min if dirs.get(l, '<') == '<' else max)(finite, key=finite.get)
            if 'SCULPTOR' in finite and best != 'SCULPTOR' and finite['SCULPTOR'] == finite[best]:
                best = 'SCULPTOR'
        for _k, disp in METHODS:
...
def main():
    ...
    ap.add_argument('--reemit-from-csv', default=None, metavar='CSV',
                    help='cosmetic regeneration: rebuild the table from a previously emitted '
                         'paper_table*.csv (means + DIRECTION row; figures/paper_artifacts is the '
                         'paper-of-record set) and re-emit .tex/.csv with the CURRENT display '
                         'rules (grid, group names/order, normalization, footnote) -- no '
                         'evaluation is run. Emits to --out and copies to --paper-dir.')
    a = ap.parse_args()
    if a.reemit_from_csv:
        return reemit_from_csv(a.reemit_from_csv, a.out, a.paper_dir)
