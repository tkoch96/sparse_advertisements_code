            ...
            for _key, disp in METHODS:
                ...
                f.write(name + ' & '
                        + ' & '.join(_fmt(c, latex=True, prec=p,
                                          with_std=False)
                                     for c, p in zip(trows[disp], tprecs))
                        + ' \\\\\n\\hline\n')
...
def reemit_from_csv(csv_path, out_dir, paper_dir):
    ...
    basename = 'paper_table_full' if 'full' in os.path.basename(csv_path) else 'paper_table'
    ...
    for l, i in zip(ordered, idx):
        col = {disp: (means.get(disp) or [None] * len(labels_csv))[i] for _k, disp in METHODS}
    ...
    emit(ordered, rows, 'all', out_dir, basename=basename)
    if basename == 'paper_table':
