...
# ----------------------------------------------------------------- rows --
METHODS = [
    # optimal reference on top (Tom 2026-08-23), then methods
...
def emit(labels, rows, fmt, out_dir):
    os.makedirs(out_dir, exist_ok=True)
    # labels are 'Group|Sub'; derive the two header rows
    groups = []
    for l in labels:
        g = l.split('|')[0]
        if groups and groups[-1][0] == g:
            ...
            groups.append([g, 1])
    subs = [l.split('|', 1)[1] for l in labels]
    ...
    if fmt in ('latex', 'all'):
        pth = os.path.join(out_dir, 'paper_table.tex')
        with open(pth, 'w') as f:
            f.write('% generated by evaluations/table_generate.py\n')
        ...
        print('\n  wrote {}'.format(pth))
    if fmt in ('csv', 'all'):
        pth = os.path.join(out_dir, 'paper_table.csv')
        with open(pth, 'w') as f:
