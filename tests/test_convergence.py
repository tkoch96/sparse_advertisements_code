...
Markers (from pytest.ini):
  ...
  integration  -- exercises Worker_Manager_ray / Ray actors.
  slow         -- can take >>1s.
...
def test_sas_solve_converges_end_to_end():
	...
	# Worker_Manager_ray needs to be picked up via the same module-aliasing
	...
	from worker_comms import Worker_Manager  # resolved via the sys.modules alias
