...
(path_distribution_computer.Path_Distribution_Computer_Actor.remote(...)) that
...
# Consolidated 2026-08-21: this file WAS worker_comms.py, fronted by a
# two-line worker_comms.py that re-exported it. The project has been
# Ray-only since the mid-2026 migration, so the split bought nothing but
# a second name for the same thing. `from core.worker_comms import
# Worker_Manager` is now the single canonical import.
...
from core.path_distribution_computer import Path_Distribution_Computer_Actor as Path_Distribution_Computer
