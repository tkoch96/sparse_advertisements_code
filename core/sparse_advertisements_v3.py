...
from core.painter import Painter_Adv_Solver
from core.anyopt import Anyopt_Adv_Solver
...
from core.deployment_setup import *
	...
	def flush_latency_benefit_queue_generic(self, **kwargs):
		...
		for worker_i in range(n_workers):
			...
			if isinstance(rets[worker_i], str):
				# The worker ships its full traceback back after the
				# 'ERROR' sentinel (2026-08-21). Print it verbatim rather
				# than truncating to 200 chars: Ray dedups worker stdout
				# across the cluster, so this is often the only surviving
				# copy of the stack that explains the failure.
				raise RuntimeError(
					'worker {} returned error for calc_compressed_lb:\n{}'.format(
						worker_i, rets[worker_i]))
			if worker_i > 0:
