...
CLI smoke (world knobs via env, same as everything else here):
    python -m core.hard_objectives --seed 1 --dpsize small
"""
import argparse
import json
import os
...
def solve_lp_frozen_failure(sas, routed_through_ingress, obj, **kwargs):
    ...
    from core.solve_lp_assignment import solve_generic_lp_with_failure_catch
    from core.static_failure_eval import (
...
# Self-registration (2026-08-21). solve_lp_assignment.py registers these from
# its tail, which works when IT is imported first. But this module moved into
# core/ (was experiments/model_error/objectives.py), so `import
# core.hard_objectives` first now hits the cycle mid-initialisation and the
# tail hook silently logs "registration FAILED" -- leaving max_util /
# frac_beyond_optimal undispatchable depending purely on import order.
# Registering here too makes it order-independent; both paths are idempotent
# dict.update calls.
if _os.environ.get('SCULPTOR_XOBJS', '0') == '1':
    try:
        register()
    except Exception as _e:  # pragma: no cover
        print('[xobjs] self-registration deferred: {}'.format(_e))
