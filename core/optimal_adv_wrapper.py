...
class Optimal_Adv_Wrapper:
	...
	def reset_link_capacities(self):
		...
		if anycast_catchments is not None: ## We possibly don't know them
			...
			from core.deployment_setup import get_link_capacities_actual_deployment
	...
	def update_deployment(self, deployment, **kwargs):
		...
		if deployment.get('dpsize') == 'small':
			...
			self.max_n_iter = 150 # maximum number of learning iterations
		env_override = os.environ.get('SCULPTOR_MAX_ITER')
		if env_override is not None:
			try:
				self.max_n_iter = int(env_override)
				print("SCULPTOR_MAX_ITER override active: max_n_iter={}".format(self.max_n_iter))
			except ValueError:
				print("WARNING: SCULPTOR_MAX_ITER={!r} is not an int; keeping default {}".format(
					env_override, self.max_n_iter))
	...
	def get_realworld_measure_wrapper(self, **kwargs):
		...
		from core.realworld_measure_wrapper import RealWorld_Measure_Wrapper
